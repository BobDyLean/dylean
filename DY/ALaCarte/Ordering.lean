...
def Array.attachWithSize
  {α: Type} [SizeOf α]
  (arr: Array α) (n: Nat) (H : sizeOf arr ≤ n := by grind) : Array {x: α // sizeOf x < n}
  :=
...
def List.attachWithSize
  {α: Type} [SizeOf α]
  (l: List α) (n: Nat) (H : sizeOf l ≤ n := by grind) : List {x: α // sizeOf x < n}
  :=
...
@[wf_preprocess]
theorem Array.compareLex_unattach {P: α → Prop} (cmp: α → α → Ordering)
...
class RepresentableOrd (f: Type → Type) [FunctorSizeOf f] [Representable f] where
  ...
  ctorDataOrd: ∀ id, Ord (Representable.ctors (f := f) id).Data
  := by
    intro id
    simp only [ALaCarte.Representable.ctors]
    cases id <;>
    exact inferInstance
  ctorDataOrd_lawfulEq: ∀ id, Std.LawfulEqOrd (Representable.ctors (f := f) id).Data
  := by
    intro id
    simp only [ALaCarte.Representable.ctors]
    cases id <;>
    exact inferInstance
  ctorDataOrd_trans: ∀ id, Std.TransOrd (Representable.ctors (f := f) id).Data
  := by
    intro id
    simp only [ALaCarte.Representable.ctors]
    cases id <;>
